...
function checkAuth(req, res) {
    const expected = storage.getAdminPassword();
    if (!expected) {
        log('error', 'ADMIN_PASSWORD non définie — espace admin désactivé', { path: req.path });
        res.status(503).json({ error: "L'espace admin n'est pas configuré sur ce déploiement." });
        return false;
    }
...
app.post('/api/config', async (req, res) => {
    if (!checkAuth(req, res)) return;
    // Sur Vercel sans store Blob, le repli fichier écrirait dans un système de
    // fichiers en lecture seule et échouerait en 500 opaque. On le dit clairement.
    if (!storage.USE_BLOB && process.env.VERCEL) {
        log('error', 'Sauvegarde impossible : store Blob absent sur Vercel');
        return res.status(503).json({
            error: "Le stockage n'est pas configuré : créez un store Blob dans le projet Vercel (Storage → Create Database → Blob).",
        });
    }
    try {
...
app.post('/api/blob/upload', async (req, res) => {
    if (!storage.USE_BLOB) {
        log('error', 'Upload impossible : BLOB_READ_WRITE_TOKEN absente');
        return res.status(503).json({
            error: "Le stockage n'est pas configuré : créez un store Blob dans le projet Vercel (Storage → Create Database → Blob).",
        });
    }
    ...
    try {
        const result = await handleUpload({
            ...
            onBeforeGenerateToken: async (_pathname, clientPayload) => {
                const expected = storage.getAdminPassword();
                if (!expected) {
                    throw new Error('ADMIN_PASSWORD non définie — upload désactivé.');
                }
